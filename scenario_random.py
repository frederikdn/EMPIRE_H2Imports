...
def _is_full_model(pathlike) -> bool:
    """Return True if the path belongs to a full_model_eur or full_model_int run."""
    s = str(pathlike)
    return ('full_model_eur' in s) or ('full_model_int' in s)
...
def sample_generator(data, regularSeasonHours, scenario, season, seasons, period, generator, sample_hour, dict_countries):
...
def sample_generator_peak(
    data, seasons, g, scenario, period, regularSeasonHours, peakSeasonHours, overall_sample, country_sample, dict_countries
):
    peak_data = pd.DataFrame()
    [country_peak, overall_peak, country_hours, overall_hours] = gather_peak_sample(
        data, seasons, regularSeasonHours, peakSeasonHours, country_sample, overall_sample
    )
    if g in ["Windoffshore", "Windoffshorenearshore", "Windoffshorefarshore", "Windoffshoregrounded", "Windoffshorefloating"]:
        startNOnode = 2
    else:
        startNOnode = 1
    for c in country_peak.columns:
...
def generate_random_scenario(scenario_data_path, tab_file_path, n_scenarios, seasons,
                             n_periods, lengthRegSeason, lengthPeakSeason,
                             regularSeasonHours, peakSeasonHours, 
                             dict_countries,  dict_offshr_nodes, LOADCHANGEMODULE,
                             filter_make, filter_use,
                             copulas_to_use,
                             copula_clusters_make, copula_clusters_use,
                             n_cluster, moment_matching, 
                             n_tree_compare, north_sea,
                             HEATMODULE = False, fix_sample = False
                             ):
    """
    ...
    :param dict_offshr_nodes: Dictionary mapping offshore nodes names
    """

    len_of_regular_season = lengthRegSeason
    len_peak_season = lengthPeakSeason
        ...
        for i in range(1, n_periods + 1):
            for scenario in range(1, n_scenarios + 1):
                for s in seasons:
                    ...
                    if _is_full_model(scenario_data_path):
                        genAvail = pd.concat(
                            [
                                genAvail,
                                sample_generator(
    ...
    genAvail = genAvail.replace({"Node": {**dict_countries, **dict_offshr_nodes}})
...
def check_scenarios_exist_and_copy(scenario_data_path, tab_file_path):
    """
    ...
    for file in scenario_files:
        if not (scenario_data_path / file).exists():
            ...
            try:
                shutil.copyfile(scenario_data_path / file, tab_file_path / file)
